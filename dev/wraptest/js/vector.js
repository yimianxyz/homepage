// --- Toroidal minimum-image displacement (EXPERIMENT: wrap-aware policy) ------
// The world wraps at the screen edges, but the predator's policy historically
// measured every distance with plain Euclidean dx=a-b. WRAP_W/WRAP_H are set to
// the canvas size when the cheap policy configures; while >0 these return the
// shortest signed displacement across the seam. Applied ONLY to predator-policy
// distances (target choice, steering, ballistic predict) — NOT to boid flocking,
// flee, or catch detection, which remain raw to mirror the fixed game rules.
var WRAP_W = 0, WRAP_H = 0;
function wrapDX(d) { var P = WRAP_W; if (P > 0) { if (d > P * 0.5) return d - P; if (d < -P * 0.5) return d + P; } return d; }
function wrapDY(d) { var P = WRAP_H; if (P > 0) { if (d > P * 0.5) return d - P; if (d < -P * 0.5) return d + P; } return d; }
