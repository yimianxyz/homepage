...
function computeEvolvedTarget(predPos, boids, opt, prevTarget) {
    ...
    for (var i = 0; i < n; i++) {
        ...
        for (var j = 0; j < n; j++) {
            var ex = wrapDX(boids[j].position.x - bxi), ey = wrapDY(boids[j].position.y - byi);
            if (ex * ex + ey * ey < R2) cnt++;
        }
        var ddx = wrapDX(bxi - px), ddy = wrapDY(byi - py);
    ...
    for (var k = 0; k < n; k++) {
        ...
        wsum += w;
        cx0 += w * wrapDX(boids[k].position.x - px); cy0 += w * wrapDY(boids[k].position.y - py);
        vx0 += w * boids[k].velocity.x; vy0 += w * boids[k].velocity.y;
    }
    if (wsum < 1e-12) wsum = 1e-12;
    cx0 /= wsum; cy0 /= wsum; vx0 /= wsum; vy0 /= wsum;   // cx0,cy0 are now RELATIVE to predator (wrap-correct)
    var nbhd = opt.nbhd || 0;
    if (nbhd > 0) {
        var bx = boids[bestIdx].position.x, by = boids[bestIdx].position.y;
        var nsum = 0, ncx = 0, ncy = 0, nvx = 0, nvy = 0;
        for (var m = 0; m < n; m++) {
            var gx = wrapDX(boids[m].position.x - bx), gy = wrapDY(boids[m].position.y - by);
            if (gx * gx + gy * gy < R2) {
                ncx += wrapDX(boids[m].position.x - px); ncy += wrapDY(boids[m].position.y - py);
                nvx += boids[m].velocity.x; nvy += boids[m].velocity.y; nsum++;
            }
        }
        if (nsum < 1e-12) nsum = 1e-12;
        ncx /= nsum; ncy /= nsum; nvx /= nsum; nvy /= nsum;   // also relative to predator
        cx0 = (1 - nbhd) * cx0 + nbhd * ncx;
        cy0 = (1 - nbhd) * cy0 + nbhd * ncy;
        vx0 = (1 - nbhd) * vx0 + nbhd * nvx;
        vy0 = (1 - nbhd) * vy0 + nbhd * nvy;
    }
    var dcent = Math.sqrt(cx0 * cx0 + cy0 * cy0);
    var lead = dcent / PREDATOR_MAX_SPEED * opt.lead_scale;
    if (lead < 0) lead = 0;
    if (lead > opt.lead_max) lead = opt.lead_max;
    var tx = px + cx0 + lead * vx0, ty = py + cy0 + lead * vy0;
