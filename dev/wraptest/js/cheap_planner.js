...
function cp_ballistic(px, py, vx, vy, bx, by, bvx, bvy, catchD, pmaxS, pmaxF, Hb) {
    ...
    for (var t = 0; t < Hb; t++) {
        var dx = wrapDX(bx - px), dy = wrapDY(by - py);
...
function cp_features(state, cands, pmaxS, pmaxF) {
    ...
    for (var k = 0; k < K; k++) {
        ...
        var rx = wrapDX(cx - px), ry = wrapDY(cy - py);
        ...
        for (var i = 0; i < n; i++) {
            var ddx = wrapDX(cx - bx[i]), ddy = wrapDY(cy - by[i]);
        ...
        var tbrx = wrapDX(tbx - px), tbry = wrapDY(tby - py);
